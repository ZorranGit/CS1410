...
class Test_add_dict_value01(unittest.TestCase):

    def test_01(self):
        for i in range(10):
            dictionary = random_dictionary()
            # get a key in the dictionary
            new_key = next(iter(dictionary.keys()))
            while new_key in dictionary.keys():
                new_key = random_key()
            new_value = random_value()
            correct_answer = dictionary.copy()
            correct_answer[new_key] = new_value
            ans = add_dict_value01(dictionary, new_key, new_value)

            msg = "add_dict_value01(%s, %s, %s) returned %s." % (str(dictionary), str(new_key), str(new_value), str(ans))
